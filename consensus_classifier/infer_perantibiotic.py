...
def run_inference(input_df, device, model, tokenizer, args):
    batch_size = 32
    pred_phenos = []
    with torch.no_grad():
...
def main():
    ...
    total_seq_true = []
    total_seq_pred = []
    per_antibiotic_acc_accuracy = {}

    #Run inference for each species
    for antibiotic, idx in antibiotic_mapping.items():
        print(f'Running inference for {antibiotic}...')
        df_antibiotic = df[df['antibiotic'] == idx]
        model, tokenizer = load_model(os.path.join(args.models_dir, antibiotic, 'best'), args, device)

        results_df = run_inference(df_antibiotic, device, model, tokenizer, args)
        ...
        if not args.final_set: 
            antibiotic_acc_accuracy = np.mean(np.array(acc_preds['pred_phenotype']) == np.array(acc_preds['true_phenotype']))
            print(f'{antibiotic} accession-level accuracy: {antibiotic_acc_accuracy:.4f}')
            per_antibiotic_acc_accuracy[antibiotic] = antibiotic_acc_accuracy

            with open(f'{args.run_name}_per_antibiotic_acc_accuracy.json', 'w') as f:
                json.dump(per_antibiotic_acc_accuracy, f)
    ...
    if not args.final_set:
        ...
        template_df = template_df.merge(
            acc_preds[['accession', 'pred_phenotype']], 
            on='accession', 
            how='left'
        )
